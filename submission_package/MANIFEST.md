...
|---|---|---|
| **Updated source files** (mandatory) | `aa52082-24_source.zip` | aanda.tex (the only .tex), aanda.bbl, cites.bib, aa.cls, aa.bst, linenoaa.sty, Figures/ (21, all used). Clean version only, per the editor's instruction. Verified to compile standalone in an empty directory: 29 pp, 0 errors, 0 undefined. |
| Latexdiff or bold pdf (optional) | `aa52082-24_marked_changes.pdf` | 30 pp. latexdiff against the round-2 submitted version (`marked_changes/old_submitted.tex`, byte-identical to the `aanda.tex` of the round-2 source zip), so it shows round-3 changes only. The round-1 baseline is kept as `marked_changes/old_submitted_round1.tex`. |
...
0 "??" in the compiled PDF. Round 3 (diff against the round-2 version): strips 0 spans, 30 pp.
