...
import erotica
sys.modules.setdefault("cosmic", erotica)  # the dill pickle predates the cosmic -> erotica rename
...
ax.axvline(best_mcs, color="steelblue", ls="--", label=f"Adopted minimum cluster size: {best_mcs}")
ax.axhline(max_size, color="olivedrab", ls="--", label=f"Maximum cluster size: {max_size}")
ax.set_xlabel("Minimum cluster size"); ax.set_ylabel("Cluster size"); ax.legend()
